...
class multi_agent_env():
    def __init__(self, n_agent):
        ...
        self.mask_dim = self.n_landmark + self.n_target
        ...
        self.action_space = gym.spaces.MultiDiscrete([self.n_action_move, self.n_landmark + self.n_target])
        self.n_action = [self.n_action_move, self.n_landmark + self.n_target]
        ...
        self.observation_space = gym.spaces.Box(low=-1, high=1,
                                                shape=[self.sensor_range ** 2 // (3 ** 2) + 3 +
                                                       # self.n_target * 1 +
                                                       self.n_landmark * 1 +
                                                       self.n_agent * 2])
    ...
    def reset(self):
        ...
        return self.obs_n(), self.get_adj()
    ...
    def step(self, action_n):
        ...
        for a_i, agent in enumerate(self.state_n):
            if action_n[a_i][0] == 5 and self.agent_monitor_n[a_i] == True and self.target_monitored_n[
        ...
        for i, action in enumerate(action_n):
            if not self.agent_monitor_n[i]:
                ...
                elif action[0] == 4 or 5:
                    new_row = self.state_n[i][0]
                    new_column = self.state_n[i][1]
                if action[1] < self.n_landmark:
                    self.agent_searching_n[i] = True
                else:
                    self.agent_searching_n[i] = False

                self.state_n[i] = np.array([new_row, new_column, action[1]])
                x_min_range = max(0, self.state_n[i][0] // (self.sensor_range - 2) - 1)
                y_min_range = max(0, self.state_n[i][1] // (self.sensor_range - 2) - 1)
                x_max_range = min(self.n_landmark_in_length - 1, self.state_n[i][0] // (self.sensor_range - 2) + 1)
                y_max_range = min(self.n_landmark_in_length - 1, self.state_n[i][1] // (self.sensor_range - 2) + 1)
                for x_i in range(x_min_range, x_max_range + 1):
                    for y_i in range(y_min_range, y_max_range + 1):
                        if np.abs(self.landmarks[y_i][x_i][0] - self.state_n[i][0]) <= (
                                self.monitor_range // 2) and np.abs(
                                self.landmarks[y_i][x_i][1] - self.state_n[i][1]) <= (self.monitor_range // 2):
                            if self.landmark_visited[y_i][x_i] == False:
                                ...
                                self.agent_score[i] += self.detected_reward * 5
        ...
        for t_i, target in enumerate(self.target_n):
            for a_i, agent in enumerate(self.state_n):
                if self.agent_monitor_n[a_i] == False:
                    if np.abs(target[0] - agent[0]) <= (self.monitor_range // 2) and np.abs(target[1] - agent[1]) <= (
                            self.monitor_range // 2):
                        if self.target_detected_n[t_i] == False:
                            self.agent_score[a_i] += self.detected_reward * 10
                            self.target_detected_n[t_i] = True
                            self.target_info[t_i] = [target[0], target[1]]
                        elif self.state_n[a_i][2] > self.n_landmark - 1 and self.stage == 1:
                            if t_i == self.state_n[a_i][2] - self.n_landmark:
                                self.agent_score[a_i] += self.detected_reward * 3

                        if action_n[a_i][0] == 4 and self.target_monitored_n[t_i] == False:  # into the monitor state
        ...
        info = {'target': self.target_info, 'landmark': info_landmark, 'state': copy.deepcopy(info_state_n)}
        ...
        if return_done and self.stage == 0:
            for landmark_visited_row in self.landmark_visited:
                for the_visited in landmark_visited_row:
                    if the_visited:
                        print("NONO", the_visited)
                        return_rew[0] += 3
                        return_rew[1] += 3
        info['rew'] = info_r
        self.pre_state_n = self.state_n
        ...
        return return_obs, return_adj, return_rew, return_done
    ...
    def get_obs(self, agent_id):
        ...
        y_min_range = max(0, self.state_n[agent_id][1] - self.sensor_range // 2 - 1)
        x_max_range = min(self.map_size - 1, self.state_n[agent_id][0] + self.sensor_range // 2 + 1)
        y_max_range = min(self.map_size - 1, self.state_n[agent_id][1] + self.sensor_range // 2)
        if x_min_range == 0 and (x_max_range - x_min_range) < self.sensor_range:
            mark_x_zero = True
        elif x_max_range == self.map_size - 1 and (x_max_range - x_min_range) < self.sensor_range:
            mark_x_edge = True
        if y_min_range == 0 and (y_max_range - y_min_range) < self.sensor_range:
            mark_y_zero = True
        elif y_max_range == self.map_size - 1 and (y_max_range - y_min_range) < self.sensor_range:
            mark_y_edge = True
        # target_view=[]
        # for y_i in range(y_min_range, y_max_range + 1):
        ...
        norm_ans_landmark_visited_times = [(1 - (value - visited_min + 0.1) / (visited_max - visited_min + 0.1)) for
        ...
        for y_i, landmarkrow in enumerate(self.landmarks):
            for x_i, thelandmark in enumerate(landmarkrow):
                self.landmarks_eye[thelandmark[1]][thelandmark[0]] = np.round(
                    5 * norm_ans_landmark_visited_times[y_i * self.n_landmark_in_length + x_i]) + 1

        landmarks_view = []  # max pooling
        for y_i in range(y_max_range, y_min_range, -1):
    ...
    def get_adj(self): # dim num_agent * num_landmarks + num_target for multipying
        adj = np.zeros((self.n_agent, self.n_landmark + self.n_target))
        for a_i in range(self.n_agent):
            x = self.state_n[a_i][0]
            y = self.state_n[a_i][1]
            for i in range(self.n_landmark):
    ...
    def reward(self):
        ...
        if self.stage == 0:
            ...
            rew += (landmark_counter - self.previous_landmark_counter) * 5
